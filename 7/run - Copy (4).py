...
def create_grid_on_map(indicator, jpgs_dir):
    ...
    if use_polygon:
        ...
        ax_poly.imshow(bg_img, extent=[0, ref_width, 0, ref_height])
        ...
        ax_preview.imshow(bg_img, extent=[0, ref_width, 0, ref_height])
        ...
        bg_path = os.path.join(jpgs_dir, "frame_0.jpg") if os.path.isfile(os.path.join(jpgs_dir, "frame_0.jpg")) else os.path.join(jpgs_dir, candidates[0])
        bg_img = Image.open(bg_path)
        ax_origin.imshow(bg_img, extent=[0, ref_width, 0, ref_height])
        ...
        plt.close(fig_origin)
        fig, ax = plt.subplots()
        ax.imshow(bg_img, extent=[0, ref_width, 0, ref_height])
...
def process_image(image_path, points):
    """
    Scale the (X, Y) from reference coordinates to the image's coordinates.
    We assume the grid is defined with top-left as (0,0) and displays the image with origin='upper'.
    """
...
def run_interactive_scatter_for_all_indicators(all_data):
    ...
    all_data: dict of indicator -> {
         'points': (pixel x_values, pixel y_values) in reference coords,
    ...
    def update_scatter():
        ...
        if d.get('geo_points'):
            ...
            # Flip x and y coordinates relative to image dimensions.
            xvals_scaled = w - (np.array(xvals) * (w / ref_width))
            yvals_scaled = h - (np.array(yvals) * (h / ref_height))
