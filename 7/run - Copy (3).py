...
#############################################
# OCR FUNCTIONS (PIL-based, without cv2)
#############################################

import pytesseract
from pathlib import Path
# Set Tesseract executable path (adjust if necessary)
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"

def pil_multi_threshold_preprocess(roi, thresholds=[80, 120, 160, 200], invert=True):
    """
    Yield preprocessed (binary) PIL images from ROI using different thresholds.
    """
    ...
    for thr in thresholds:
        ...
        yield bin_img
        if invert:
            inv_img = ImageOps.invert(bin_img)
            yield inv_img

def pil_attempt_ocr(preprocessed_images, config):
    """
    Run OCR on an iterable of PIL images and return the first recognized date.
    """
...
def extract_date(image_path):
    """
    Attempt to extract a date (YYYY-MM-DD) from an image using PIL.
    Two regions are used:
      1. Dynamic region: top-right corner (approximately 15% height, last 25% width)
      2. Fallback fixed region: a 160x60 pixel region in the top-right corner.
    Multiple thresholds and inversion are applied.
    """
    ...
    # Dynamic region (adjust percentages as needed)
    roi_dyn = img.crop((int(0.75 * width), 0, width, int(0.15 * height)))
    # Resize ROI to improve OCR resolution
    ...
    if date_fixed:
        ...
        print(f"Date not found in both dynamic and fixed attempts for {Path(image_path).name}")
        return None
...
def read_dates_for_jpgs(jpgs_dir):
    ...
    frame_dates = [None] * (data[-1][0] + 1 if data else 0)
    for (fnum, dt) in data:
        if 0 <= fnum < len(frame_dates):
            frame_dates[fnum] = dt
    return frame_dates
...
#############################################
# MAIN PROCESSING FUNCTIONS (Parts A, C, D, E)
#############################################
...
def run_partA(input_grid_folder, input_images_folder, output_folder, indicator):
    ...
    jpg_files = [os.path.join(input_images_folder, f)
                 for f in os.listdir(input_images_folder) if f.lower().endswith('.jpg')]
    if not jpg_files:
...
def run_interactive_scatter_for_all_indicators(all_data):
    ...
    all_data: dict of indicator -> {
         ...
         'results_file': path to results_{indicator}.xlsx
    }
    """
    if not all_data:
    ...
    ax1.callbacks.connect('xlim_changed', lambda ax: None)
    ax1.callbacks.connect('ylim_changed', lambda ax: None)
    ax2.callbacks.connect('xlim_changed', lambda ax: None)
    ax2.callbacks.connect('ylim_changed', lambda ax: None)
    ...
    # --- Show Evolution: Single plot with color strips for first & last grid points, annotated with dates ---
    ax_line = plt.axes([0.05, 0.05, 0.15, 0.05])
    btn_line = Button(ax_line, 'Show Evolution')

    def show_evolution(event):
        # Optionally, let the user draw a line (for context)
        ...
        if d.get('geo_bounds'):
            ...
            transformer = Transformer.from_crs("EPSG:4326", "EPSG:3857", always_xy=True)
            pts_proj = []
            for x, y in zip(d['points'][0], d['points'][1]):
                lon = lon_min + (x/512) * (lon_max - lon_min)
                lat = lat_min + (y/512) * (lat_max - lat_min)
                try:
                    x_proj, y_proj = transformer.transform(lon, lat)
                    pts_proj.append([x_proj, y_proj])
                except Exception as ex:
                    print("Error transforming point:", ex)
            pts_arr = np.array(pts_proj)
        else:
            transformer = Transformer.from_crs("EPSG:4326", "EPSG:3857", always_xy=True)
            pts_proj = []
            for lon, lat in zip(d['points'][0], d['points'][1]):
                try:
                    x_proj, y_proj = transformer.transform(lon, lat)
                    pts_proj.append([x_proj, y_proj])
                except Exception as ex:
        ...
        if pts_arr.shape[0] == 0:
            print("No grid points available.")
            return
        if pts_arr.shape[0] >= 2:
            selected_indices = [0, pts_arr.shape[0]-1]
        else:
            selected_indices = [0]
        print("Selected grid point indices:", selected_indices)
        ts_dict = get_time_series_for_points(d.get('results_file'), selected_indices)
        ...
        fig2, ax_cstrip = plt.subplots(figsize=(12, 2*num_points))
        ax_cstrip.imshow(combined_data, aspect='auto')
        ax_cstrip.set_title("Grayscale Variation for First & Last Grid Points")
        ax_cstrip.set_ylabel("Grid Points")
        ax_cstrip.set_xlabel("Frame (or Date)")
        ax_cstrip.set_yticks(range(num_points))
        ax_cstrip.set_yticklabels([f"Point {idx}" for idx in selected_indices])
        guess_jpgs = None
        ...
        if guess_jpgs and os.path.isdir(guess_jpgs):
            frame_dates = read_dates_for_jpgs(guess_jpgs)
        n_frames = max_frames
        if n_frames <= 30:
            step = 1
        else:
            step = max(1, n_frames // 10)
        tick_positions = range(0, n_frames, step)
        tick_labels = []
        for f_i in tick_positions:
            if frame_dates and f_i < len(frame_dates) and frame_dates[f_i]:
                tick_labels.append(frame_dates[f_i].strftime('%m-%d'))
            else:
                tick_labels.append(str(f_i))
        ax_cstrip.set_xticks(tick_positions)
        ax_cstrip.set_xticklabels(tick_labels, rotation=45, ha='right')
        plt.tight_layout()
        plt.show()
...
def main_option2_single_window():
    ...
    for indicator in valid_folders:
        ...
        if kml_bounds:
            ...
            geo_x_vals = [lon_min + (xx/512)*(lon_max - lon_min) for xx in x_vals]
            geo_y_vals = [lat_min + (yy/512)*(lat_max - lat_min) for yy in y_vals]
        ...
        try:
            ...
            continue
        frame0 = os.path.join(jpgs_sub, 'frame_0.jpg')
        if not os.path.isfile(frame0):
            cand = [f for f in os.listdir(jpgs_sub) if f.lower().endswith('.jpg')]
            if cand:
                frame0 = os.path.join(jpgs_sub, cand[0])
            else:
                logging.error(f"No background image in {jpgs_sub}. Skipping {indicator}.")
                continue
